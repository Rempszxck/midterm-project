  ...
  return (
    <div className="App">
      ...
      <div className={`search-wrapper ${isActiveSearch ? 'search-top' : 'search-center'}`}>
        <div>
          <label htmlFor="search" className="search-label">Search a Country</label>
        </div>
        ...
        <input
          ...
          style={{ backgroundColor: plainMode ? 'green' : 'red' }}
          value={plainMode ? 'Plain Mode' : 'JSON Mode'}
          // onClick={() => setPlainMode(!plainMode)}
          onClick={() => {
            setPlainMode(!plainMode);
            console.log('Toggle Plain Mode:', !plainMode);
          }}
          checked={plainMode}
